...
/** SPA bootstrap.
  *
  * Vite/Scala.js calls `Main.main(Array.empty)` once when the bundle loads.
  * We grab `<div id="root">` (defined in `client/index.html`) and create a
  * single React root that owns the entire SPA tree. Everything else —
  * routing, page lookup, layout — lives behind [[Router.Component]].
  *
  * Look at this file second when onboarding (after `Router.scala`); it's
  * intentionally tiny so the routing surface stays the obvious entry.
  */
object Main:
