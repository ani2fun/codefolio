...
/** Lists every book in the Cortex. Each card links to `/cortex/<slug>`,
  * which the BookRedirectPage forwards to the first chapter.
  */
object CortexIndexPage:

  private type Loaded = Either[String, CortexIndex]
  ...
  val Component =
    ScalaFnComponent
      ...
      .useEffectOnMountBy { (_, state) =>
        PageTitle.set("Cortex — Aniket Kakde") >> Callback {
