...
/** Lists every book in the knowledge base. Each card links to
  * `/knowledge/<slug>`, which the BookRedirectPage forwards to the first
  * chapter.
  */
object KnowledgeIndexPage:

  private type Loaded = Either[String, KnowledgeIndex]

  val Component =
    ScalaFnComponent
      .withHooks[Unit]
      .useState(Option.empty[Loaded])
      .useEffectOnMountBy { (_, state) =>
        PageTitle.set("Knowledge Base — Aniket Kakde") >> Callback {
        ...
        <.main(
          ...
          <.section(
            ...
            state.value match
              ...
              case Some(Left(msg)) =>
                <.p(^.className := "text-center text-destructive", msg)
              case Some(Right(idx)) =>
                if idx.books.isEmpty then
                  <.p(
