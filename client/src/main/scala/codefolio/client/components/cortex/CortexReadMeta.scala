 ...
 *
 * Per chat-4 user feedback the time-based signals ("4 min read", percentage indicators) were dropped —
 * the reader shouldn't feel pressure to finish in a specific time. Word count + author still help frame
 * the chapter (length expectation, attribution) without putting a clock on the user.
 */
object CortexReadMeta:

  private val Author = "Aniket Kakde"

  /** Compute words from the raw markdown body. Whitespace split; markdown punctuation is irrelevant at
    * this resolution (the difference between 1240 and 1257 words is rounding noise to the reader). */
