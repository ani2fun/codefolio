...
/**
 * Right-edge vertical mini-map: a thin track with one tick per H3 in the chapter prose, a fill that
 * rises with scroll, and labels that surface on tile hover. Click a tick to jump.
 *
 *   - Ticks are placed proportionally to each H3's vertical position inside `.chapter-content` at
 *     mount, and rebuilt on resize and when the type-controls dispatch `cortex:typePrefsChanged`
 *     (prose height changes with font size).
 *   - Fill height tracks `ReaderState.currentScrollFraction()` on every scroll.
 *   - Active tick mirrors the right-TOC's active heading via the same `cortex:activeHeading` event the
 *     sticky bar listens to.
...
object CortexMiniMap:
  ...
  /** Wait for ChapterContent to mount its dangerouslySetInnerHTML payload before measuring headings.
    * 250ms is comfortably longer than React's commit + the next browser paint, but well below the
    * point where a user notices the mini-map appearing. */
            ...
            <.div(
              ...
              ticks.toTagMod { tick =>
                ...
                <.button(
                  ...
                  ^.style := js.Dynamic.literal(top = s"${(tick.topPct * 100).max(0.0).min(100.0)}%").asInstanceOf[js.Object],
