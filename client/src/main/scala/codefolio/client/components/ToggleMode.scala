...
/** Dark/light theme toggle button. Mirrors portfolio-app's ToggleMode:
  ...
  *   - On the very first render we show a disabled placeholder, since the
  *     `<html>` class might not yet be observable until the inline-script
  *     bootstrap has run; this mirrors the `mounted` guard in the original.
  *
  * Local component state mirrors `Theme.current` and is updated in-step with
  * `Theme.toggle`, which keeps this component re-rendering in sync with the
  * actual <html> class. Components elsewhere that depend on the theme should
  * rely on Tailwind's `dark:` selectors and re-render via their own state.
  */
object ToggleMode:
