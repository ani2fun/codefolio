...
2. **Open a chapter** — GET `/api/knowledge/{book}/{chapter}` then run the markdown pipeline locally. Two-stage loading.
...
sequenceDiagram
  ...
  U->>Br: paste/reload<br/>/knowledge/distributed-systems/introduction
  Br->>Sv: GET /knowledge/...
  Sv-->>Br: index.html (SPA fallback)
  Br->>Ro: window.location parsed
  Ro->>Cp: render ChapterPage(book, chapter)
  Cp->>Ac: getKnowledgeChapter(book, chapter)
  Ac->>Sv: GET /api/knowledge/distributed-systems/introduction
  Sv->>Kh: tapir routes
  Kh->>Kh: read meta.json + chapter.md (path-traversal guard)
...
| --- | --- |
| 404 on a known chapter | Filename ≠ `meta.json` slug; or the file isn't in `meta.json` at all |
...
- **No DB writes from production traffic.** The portfolio sections read from bundled JSON; the knowledge base reads from disk. Postgres / Redis / Mongo are exercised by the **Hello demo** at `/demo` only — kept around because they're useful smoke tests of the persistence layer.
