...
| `client/index.html`, `client/tailwind.css` | Vite HMR |
...
| `content/knowledge/**/*.md` | served from disk by the running server — refresh the browser |
...
| `KNOWLEDGE_ROOT` | `KnowledgeHandler.scala` | `./content/knowledge` (dev) / `/app/content/knowledge` (Docker) | where books live |
...
curl http://localhost:8080/api/knowledge/index | jq .      # all books at a glance
```
