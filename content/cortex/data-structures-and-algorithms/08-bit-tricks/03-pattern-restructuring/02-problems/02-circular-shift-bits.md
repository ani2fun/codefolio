...
> 🖼 Diagram — Left rotation: combine the leftshift (which drops top bits) with a rightshift of complementary distance (which extracts those same top bits and lands them at the bottom). OR the two together for a lossless rotate.
```mermaid
---
config:
  theme: base
