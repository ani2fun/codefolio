...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < V; i++) Solution.adj.add(new ArrayList<>());
        for (int[] c : clauses) {
            Solution.adj.get(Solution.neg(Solution.toV(c[0]))).add(Solution.toV(c[1]));
            Solution.adj.get(Solution.neg(Solution.toV(c[1]))).add(Solution.toV(c[0]));
        }
        Solution.disc = new int[V]; Solution.low = new int[V]; Solution.sccId = new int[V]; Solution.onStack = new boolean[V];
        Arrays.fill(Solution.disc, -1);
        Solution.stk = new ArrayDeque<>();
        for (int v = 0; v < V; v++) if (Solution.disc[v] == -1) Solution.tarjan(v);

        boolean sat = true;
        boolean[] assignment = new boolean[Solution.n];
        for (int i = 0; i < Solution.n; i++) {
            if (Solution.sccId[2*i] == Solution.sccId[2*i + 1]) { sat = false; break; }
            assignment[i] = Solution.sccId[2*i] < Solution.sccId[2*i + 1];
        }
        if (sat) for (int i = 0; i < Solution.n; i++) System.out.println("x" + (i+1) + " = " + assignment[i]);
...
object Main extends App {
  ...
  Solution.solve2sat(n, clauses) match {
