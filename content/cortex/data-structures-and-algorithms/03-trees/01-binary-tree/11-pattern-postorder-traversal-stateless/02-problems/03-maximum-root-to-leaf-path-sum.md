...
> 🖼 Diagram — Max path sum — each node returns its own value plus the better of the two subtree answers. Empty subtrees contribute 0; the recursion bubbles the maximum up to the root.
```mermaid
---
config:
  theme: base
