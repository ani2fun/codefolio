...
> 🖼 Diagram — K-way merge with a sliding window. The min-heap tracks the smallest, an external maxValue tracks the largest, and their difference is the current candidate range.
```mermaid
---
config:
  theme: base
