...
In production, **Raft has effectively won**. The few well-known Paxos systems (Google's Chubby, Spanner's TrueTime layer) predate Raft; new systems pick Raft. Aspects of Paxos — `Fast Paxos`, `Egalitarian Paxos`, `Mencius` — are still active research for specific performance tweaks.
...
> **Next:** with this, **Part 2 Building Blocks is complete.** The next stretch of the curriculum is Part 3 Distributed Patterns — messaging, idempotency, sagas, rate limiting, circuit breakers — and the capstones that compose everything you've learned into full system designs. The first capstone, [37 — URL shortener](/cortex/system-design/capstones-url-shortener), is the dry run of the capstone format: it reuses every widget in this part to walk one end-to-end design.
