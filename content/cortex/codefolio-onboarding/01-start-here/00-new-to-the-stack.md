...
2. **[Hello World, End to End](./hello-world-end-to-end)** — the single best chapter to start with. It traces one real request through every layer, so the abstract pieces above become concrete.
3. **[Overview](./overview)** and **[Repository Tour](./repository-tour)** — the big-picture map.
4. **[Local Development](./local-development)** — get it running on your machine.
...
Don't try to master these first. Get the app running ([Local Development](./local-development)), read the Hello World trace, and learn each library the moment you first need it.
