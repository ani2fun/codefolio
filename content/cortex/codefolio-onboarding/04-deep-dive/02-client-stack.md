...
This chapter is the client-side companion to [Server Stack](./server-stack). Same format: per-decision, **what we use**, **why this and not the obvious alternative**, **what breaks if you change it**. If you're new to Scala.js, the section on the hook builder is the one to internalise — every component in the codebase follows the same pattern.
...
1. **One language, two runtimes.** The same `RunRequest(language, source, stdin)` case class lives on both sides. Edit `api/openapi.yaml`, recompile, and both server and client break in step (see [Shared & Codegen](./shared-and-codegen)). No copy-paste, no DTO drift.
...
Two-step rendering, fully explained in [Markdown Pipeline](./markdown-pipeline) but worth the deep dive here:
