...
The trade-off vs zio-json: zio-json isn't supported by the codegen plugin at all. We could hand-write zio-json codecs, but then we'd lose the auto-generated ones, which is the whole point of the plugin. (Discussed in [Server Stack](./server-stack#why-circe-and-not-zio-json).)
...
This is also why the [Extending the Project](./extending) recipe for "add an API endpoint" insists on starting at `api/openapi.yaml` and chasing compile errors — that's the codegen safety net at work.
