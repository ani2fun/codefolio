...
| `sbt-openapi-codegen` | The codegen pipeline (see [Shared & Codegen](./shared-and-codegen)). Reads `api/openapi.yaml`, emits Scala. |
...
Already covered in [Client Stack](./client-stack#bundle-splitting), but worth repeating because the trade-off is the entire reason the home page loads fast:
...
The annoying part: **Liquibase logs through `java.util.logging`**, and sbt-revolver tags everything on stderr as `[ERROR]`. We bridge JUL→SLF4J in `Main.scala` (`SLF4JBridgeHandler.install()`) — without that bridge, every dev startup looks alarming. Covered in [Server Stack](./server-stack#liquibase-yaml--julslf4j-bridge).
