...
The closed vocabulary of marker (pointer) names + their fixed role colours that the `linked-list` D3 widget enforces — `head`/`tail`/`prev`/`curr`/`next`/`slow`/`fast`/`dummy`/`start`/`end`. The widget rejects any name outside this set with an inline `⚠ name` warning badge in place of the usual triangle + label, and strips author-provided `color` fields (the colour is resolved from the canon, not the payload). Pedagogical context that previously crept into marker names (`previous=null`, `new head`, `stitched`) moves into the per-step `msg` caption. The node-style canon is `{new, removed, highlight}`; unknown styles drop with a dev-mode console warning. See ADR-0016. Growing the canon is an ADR amendment, not an ad-hoc payload field.
