...
**Stepper**:
The shared play-loop hook `client.components.cortex.widgets.Stepper` used by D3 Widgets that animate over discrete steps — currently **Array Traversal Stepper**, **Linked-list widget**, and **Partition Simulator**. Owns the step index, the `playing` flag, the timeout id ref, and the play-loop `useEffect`; widgets pass `Stepper.Input(stepCount, delayMs)` and consume `Stepper.Output(index, isPlaying, atStart, atEnd, previous, next, reset, togglePlay, jumpTo)` to render their own SVG and per-widget control bar. The hook is the **interface**; the play-loop, timeout cleanup, and step-clamp are the **implementation** behind it. A widget with a speed-multiplier UI (linked-list) owns the speed state and feeds the resulting `delayMs` back in; the hook reschedules the timer immediately because `delayMs` is in its effect's dep tuple. Widgets that don't animate over discrete steps don't use Stepper — slider-driven simulators (Queueing Simulator) and phase-ADT widgets (Traced Code) keep their own state shape. The control-bar VDOM stays in each widget because BEM classes + speed pickers + section dividers are widget-specific styling; the hook deliberately doesn't return JSX.
_Avoid_: Step Controller, Step Driver, Play-loop Hook (the module is literally `Stepper`; CONTEXT term tracks the code name).

**Payload Decoder**:
The shared JSON-payload decoder `client.components.cortex.widgets.PayloadDecoder` used by every D3 Widget to parse the JSON string handed to it via `Block.D3Widget(name, payload)`. Owns the `js.JSON.parse → Try → Either[String, T]` envelope (`PayloadDecoder.run`), typed extension methods on `js.Dynamic` for required and optional field reads (`string`, `int`, `double`, `bool`, `optString`, `optInt`, `optObj`, `dynList`, `stringList`, `intRange`, `doubleRange`), plus exception builders (`missing(name)`, `invalid(message)`) that widgets throw from inside the decoder lambda to mark a field absent or a validation failure — `run` catches and folds them into the renderer-visible `Left(message)`. Each widget keeps its schema (`final case class Spec(...)`) and the decoder body that translates `js.Dynamic` → `Spec`; the **mechanics** of unsafe JS interop, the `Try → Either` envelope, and the empty-payload error message live once. Adding a new widget is one decoder lambda, not a 50-line `js.UndefOr[T].toOption.getOrElse(default)` ladder. Widgets that previously held identical local `parseRange` / `parseIntRange` / `parseDoubleRange` helpers now call `d.intRange(name, default)` and `d.doubleRange(name, default)` directly.
_Avoid_: Payload Parser, Widget Decoder, JS-Interop Helpers (the module is literally `PayloadDecoder`; CONTEXT term tracks the code name).
...
**Linked-list Marker Canon**:
The closed vocabulary of marker (pointer) names + their fixed role colours that the `linked-list` D3 widget enforces — `head`/`tail`/`previous`/`current`/`next`/`slow`/`fast`/`dummy`/`start`/`end` plus `headA`/`headB`/`headC`/`tailA`/`tailB`/`tailC` for multi-list operations. Names match the natural variable names in the source's algorithm code (`previous`, `current` — not the terser `prev`, `curr`) so the diagram label, the prose, and the runnable code all read the same. The widget rejects any name outside this set with an inline `⚠ name` warning badge in place of the usual triangle + label, and strips author-provided `color` fields (the colour is resolved from the canon, not the payload). Pedagogical context that previously crept into marker names (`previous=null`, `new head`, `stitched`) moves into the per-step `msg` caption. The node-style canon is `{new, removed, highlight}`; unknown styles drop with a dev-mode console warning. See ADR-0016. Growing the canon is an ADR amendment, not an ad-hoc payload field.
_Avoid_: Marker Vocabulary, Pointer Names (too generic — the *hard enforcement* + role-bound colours are the defining traits).

**Diagram Format Decision Rule**:
The rule for picking which Block format a chapter diagram should use — **D3 widget** for dynamic state over time (insert/delete/reverse/two-pointer walks), **D2 static block** for structural figures (node anatomy, memory layout, single annotated snapshot), **Mermaid flowchart** for algorithm control flow (`init → loop → end`, decision branches). A chapter may use all three, but never two formats for the same content. See ADR-0015. The widget catalog (ADR-0006), animation runtime (ADR-0013), and per-widget ADRs are unchanged — this is a *content* rule that applies above the rendering layer.
_Avoid_: Diagram Picker, Format Guide (these are too process-y; this is a static decision table, not a workflow).
...
**Visualise**:
The trace-driven interactive diagram opened from a runnable Python block's **Visualise** button. The author marks the fence ` ```python run viz=<layout> viz-root=<var> `; `VisualiseModal` traces the *real* solution with the shared `PythonTracer` `sys.settrace` harness, reconstructs each step's heap into a `VizGraph` with the data-structure-agnostic `shared.viz.HeapToGraph` adapter, and animates it with one generic pure-D3 renderer (`client/src/d3/`) — code stepping line-by-line beside the diagram. There is **no per-algorithm code**: adding a data structure adds a ~50-line layout, never an engine or a renderer. See ADR-0018. The pilot was then polished into a proven reference (ADR-0019) — pointers coloured by *role* (the shared `MarkerColors` canon), calm arrowheaded edges, `changed` / `removed` node states from the heap diff, generated narration captions, and builder / constructor frames dropped so the trace steps through the algorithm with every line its own step. Shares the harness with **Traced Code** (` ```python trace `) but does a different job: Traced Code shows code + a locals panel; Visualise reconstructs and animates the *data structure*.
_Avoid_: Diagram, Animation, Trace Diagram (too generic — the defining traits are the heap reconstruction and the no-per-algorithm-code / one-generic-renderer invariant).
