...
import codefolio.server.handlers.{CodeRunHandler, CortexFailure, CortexHandler, HelloHandler, RunFailure}
 ...
 *
 *   1. **API endpoints (tapir)** — typed `ZServerEndpoint`s built from the codegen'd `Endpoints` values. Each
 *      `xxxEndpoint` below pairs a generated description with a handler call and an error mapping. The full
 *      list goes into `apiEndpoints`, then through `ZioHttpInterpreter` to become `Routes[Any, Response]`.
 *
 * 2. **Static-file routes** — index.html, `/assets/`, `/img/`, etc. Mounted only when `STATIC_DIR` actually
 * exists on disk (so the dev server doesn't shadow Vite's :5173).
 *
 * 3. **SPA fallback** — known top-level client routes (/cortex, /demo, /blogs, ...) all serve `index.html` so
 * a hard reload of a deep-link works. We list them explicitly rather than using a single wildcard — see the
 * long comment further down for why.
 *
 * If you're adding an endpoint, the recipe is in the Onboarding book (`Extending the Project`, Recipe 2). The
 * short version: define the endpoint in `api/openapi.yaml`, write a handler, then add a `ZServerEndpoint`
 * here and append it to `apiEndpoints`.
 */
trait HttpApp:
  def serve: Task[Unit]
...
object HttpApp:
  ...
  val live: ZLayer[AppConfig & HelloHandler & CodeRunHandler & CortexHandler, Nothing, HttpApp] =
...
final private class HttpAppLive(
  ...
  // Cortex endpoints. Errors map to 404 (not found / traversal) or 500
  // (IO / corrupt index). All pair the status code with an ApiError body
  // for consistency with /api/run.
  private val cortexIndexEndpoint: ZServerEndpoint[Any, Any] =
    Endpoints.getCortexIndex
      .errorOut(statusCode and jsonBody[ApiError])
      .zServerLogic { _ =>
        cortex.index.mapError(failureToApiError)
      }

  private val cortexChapterEndpoint: ZServerEndpoint[Any, Any] =
    Endpoints.getCortexChapter
      .errorOut(statusCode and jsonBody[ApiError])
      .zServerLogic { case (book, chapter) =>
        cortex.chapter(book, chapter).mapError(failureToApiError)
      }

  private def failureToApiError(f: CortexFailure): (StatusCode, ApiError) = f match
  ...
  private def trailingFileHandler(prefix: String)
  ...
  private val staticRoutes: Routes[Any, Response] =
    if !staticDirExists then Routes.empty
      ...
      Routes(
        ...
        Method.GET / "cortex" -> handler(staticIndex),
        Method.GET / "cortex" / trailing ->
