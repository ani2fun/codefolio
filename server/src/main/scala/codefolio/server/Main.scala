...
import codefolio.server.handlers.{CodeRunHandler, HelloHandler, KnowledgeHandler}
...
/** Server entry point.
  ...
  *   Main.run
  *     │
  *     ├── bootstrap: install Typesafe config provider so `application.conf`
  *     │             (and env-var overrides) feed `AppConfig.live`.
  *     │
  *     ├── Migrations.run (Liquibase against Postgres)
  *     │
  *     └── HttpApp.serve  (binds the port, blocks until shutdown)
  *
  * `provide(...)` wires the **dependency layers** that produce each service
  * from raw config + the JVM. ZIO assembles them into a DAG; if any layer
  * is missing the compiler complains with a "missing service" error before
  * the process ever starts. Order in the `provide` list does not matter.
  *
  * If you add a new service:
  *   1. Define it as `trait MyThing` + `object MyThing { val live: ZLayer[..., MyThing] }`.
  *   2. Append `MyThing.live` to the `provide` list below.
  *   3. Inject it into whichever layer needs it (typically `HttpApp.live`).
  */
  ...
  override def run: ZIO[Any, Throwable, Unit] =
    ...
    program
      .provide(
        ...
        KnowledgeHandler.live, // /api/knowledge/*
