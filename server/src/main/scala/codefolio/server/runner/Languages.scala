...
/** The set of languages we expose as runnable code blocks.
  *
  * The numeric `id` field is the canonical identifier and matches Judge0's
  * submission language IDs (kept as the canonical key so the same enum
  * works for both Piston and the local Code Runner container, which speaks
  * the Judge0 submissions API protocol). Aliases let the markdown layer
  * (and the user) write `python` or `py` without caring about the IDs.
  *
  * Mirror of `portfolio-app/src/lib/judge0.ts#RUNNABLE_LANGUAGES`. SQL is
  * intentionally absent from Piston (no sqlite runtime) but present here —
  * the handler will reject it cleanly when only Piston is configured.
  */
object Languages:
  ...
  val all: List[RunnableLanguageInfo] = List(
    ...
    RunnableLanguageInfo(id = 63, label = "JavaScript (Node.js 12)", aliases = Seq("javascript", "js", "node")),
  ...
  /** Resolve an alias (case-insensitive). Returns `None` for unknown
    * languages so the handler can return a 400.
    */
