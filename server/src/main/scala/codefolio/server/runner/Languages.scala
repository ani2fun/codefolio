 ...
 *
 * Mirror of `portfolio-app/src/lib/judge0.ts#RUNNABLE_LANGUAGES`. SQL is intentionally absent from Piston (no
 * sqlite runtime) but present here — the handler will reject it cleanly when only Piston is configured.
 */
object Languages:
  ...
  /**
   * Resolve an alias (case-insensitive). Returns `None` for unknown languages so the handler can return a
   * 400.
   */
